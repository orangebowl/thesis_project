...
class RBFPOUNet:
    ...
    def forward(params, x):
        ...
        # Numerically stable implementation of RBF
        log_phi = -d2 / (w**2 + 1e-12)
        log_phi = log_phi - jnp.max(log_phi, 1, keepdims=True) # Stabilize
        phi = jnp.exp(log_phi)
        phi = jnp.maximum(phi, 1e-6) # Avoid collapse
        return phi / jnp.sum(phi, 1, keepdims=True)

class WindowModule(eqx.Module):
    """一个Equinox模块，用于封装训练好的PoU网络，使其能作为FBPINN的窗函数"""
    pou_net: Any
    params: Optional[Dict] = eqx.static_field(default=None)
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        return self.pou_net.forward(self.params, x)

# ==============================================================================
# 3. 局部多项式拟合与PoU训练 (Two-Phase LSGD)
# ==============================================================================
...
def fit_local_polynomials(x, y, w, lam: float = 0.0):
    A, y_reshaped = _design_matrix(x), y[:, None]; k = A.shape[-1]
    # Use vmap for batch processing of local fits
    def _solve(weights):
        Aw = A * weights[:, None]
        M = A.T @ Aw
...
def _predict_from_coeffs(x, coeffs, partitions):
    A = _design_matrix(x)
    y_cent = A @ coeffs.T # Predictions from each local model
    return jnp.sum(partitions * y_cent, 1)

@dataclasses.dataclass
class TwoPhaseLSGDConfig:
    """Configuration for the Two-Phase LSGD trainer."""
    n_epochs_p1: int = 5000
    lr_p1: float = 0.01
    n_epochs_p2: int = 3000
    lr_p2: float = 0.005
    lam_init: float = 0.1
    rho: float = 0.99
    n_stag: int = 100

def run_two_phase_lsgd(pou_net, initial_params, x, y, cfg: TwoPhaseLSGDConfig):
    """
    Trains the PoU network using a two-phase Locally Supported Gradient Descent (LSGD) approach.
    Phase 1: Regularized training to establish good partition coverage.
    Phase 2: Fine-tuning without regularization for optimal fit.
    """
    ...
    def loss_fn(p, lam):
        ...
        # The loss is the mean squared error of the reconstruction
    ...
    for phase in range(1, 3):
        n_epochs = cfg.n_epochs_p1 if phase == 1 else cfg.n_epochs_p2
        lr = cfg.lr_p1 if phase == 1 else cfg.lr_p2
        # Regularization is only active in Phase 1
        lam = jnp.array(cfg.lam_init) if phase == 1 else jnp.array(0.0)
        
        opt = optax.adam(lr)
        opt_state = opt.init(params)
        
        print(f"\n--- PoU Training: Phase {phase} (Epochs: {n_epochs}, LR: {lr}, Lambda_init: {lam:.2e}) ---")
        best_loss, stag_counter = jnp.inf, 0
        bar = trange(n_epochs, desc=f"PoU-LSGD (Phase {phase})", dynamic_ncols=True)
        ...
        for ep in bar:
            ...
            bar.set_postfix(loss=f"{loss_val:.3e}", lam=f"{lam:.2e}")

            if loss_val < best_loss - 1e-12:
                best_loss, stag_counter = loss_val, 0
            else:
                stag_counter += 1
            
            # Decay regularization lambda in Phase 1 if loss stagnates
            if phase == 1 and stag_counter > cfg.n_stag:
                lam *= cfg.rho
                stag_counter = 0
                print(f"\nStagnation detected. Lambda decayed to {lam:.4f}")
...
def train_fbpinn_simple(key, model, problem, colloc, lr, steps, x_test, u_exact, eval_every=100):
    ...
    if not is_real_window:
        ...
        # 1. Create a pytree of Falses with the same structure as the model.
        where_false = jax.tree.map(lambda _: False, model)
        # 2. Create a pytree of Trues with the same structure as the part to freeze.
        where_true_for_window = jax.tree.map(lambda _: True, model.window_fn)
        # 3. Create the valid filter spec by placing the Trues at the correct location.
        filter_spec_for_frozen = eqx.tree_at(lambda m: m.window_fn, where_false, where_true_for_window)
        # --- END: CORRECTED CODE ---

        # This partition will now work correctly.
        ...
        def final_model_builder(p, s):
            # Recombine the trained part with the frozen part
...
if __name__ == '__main__':
    ...
    POU_CONFIG = TwoPhaseLSGDConfig(
        n_epochs_p1=5000, lr_p1=0.01,
        n_epochs_p2=3000, lr_p2=0.005,
        lam_init=0.1, rho=0.99, n_stag=100
    )
    ...
    plot_results(current_model, problem, x_test, "Initial (n_sub=1)", 1, save_dir)
    plot_loss_history(loss_hist, l1_steps, l1_hist, "Initial (n_sub=1)", 1, save_dir)
    ...
    while current_n_sub < N_max:
        ...
        print(f"-- 正在为 {next_n_sub} 个分区训练PoU模型 --")
        ...
        # Call the new two-phase trainer
        final_pou_params = run_two_phase_lsgd(pou_net, pou_net.init_params(), colloc_train, y_train_pou, POU_CONFIG)
        ...
        mlp_conf = dict(in_size=problem_dim, out_size=1, **BASE_MLP_SMALL)
    ...
    # 5. 保存和总结
    final_l1_steps, final_l1_hist = l1_steps, l1_hist # from the last training run
