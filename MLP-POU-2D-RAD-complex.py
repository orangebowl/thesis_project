...
HEXAGON_RADIUS   = 1.0      # 外接圆半径
MESH_RESOLUTION  = 64       # mshr 网格分辨率，越大网格越密
phase_shift      = np.pi/2  # 让一个顶点正对 y 轴



def solve_poisson_fem_hexagon_fenics(resolution: int = 64,
                                     grid_res: int = 201,
                                     k_min: float = 2.0,
                                     k_max: float = 10.0,
                                     omega: float = np.pi,
    ...
    # ------------ 1. 生成“尖角朝上”六边形网格 -----------------------
    R = 1.0
    phase = np.pi/2                       # 旋转 π/2
    verts  = [Point(R*np.cos(k*2*np.pi/6 + phase),
                    R*np.sin(k*2*np.pi/6 + phase))
              for k in range(6)]
    ...
    for i in range(grid_res):
        for j in range(grid_res):
            p = Point(float(X[i, j]), float(Y[i, j]))
            if domain.inside(p):                # mshr.Polygon.inside(Point)
                U[i, j] = u_sol(p)

    # ------------ 6. 内部节点坐标及对应解 --------------------------
    coords   = mesh.coordinates()
    mask_int = np.array([domain.inside(Point(*pt)) for pt in coords])
    pts_int  = coords[mask_int]
    u_flat   = u_sol.vector().get_local()[mask_int]

    return U, X, Y, pts_int, u_flat

PHASE_SHIFT = np.pi / 2.0        # 顶点正对 y 轴

HEXAGON_VERTICES = jnp.array([
    [HEXAGON_RADIUS * jnp.cos(theta + PHASE_SHIFT),
     HEXAGON_RADIUS * jnp.sin(theta + PHASE_SHIFT)]
    for theta in jnp.linspace(0, 2 * jnp.pi, 7)[:-1]
])
...
def is_inside_hexagon(xy: jnp.ndarray) -> jnp.ndarray:
    ...
    signs = jnp.zeros(x.shape[0], dtype=jnp.bool_)
    for i in range(len(HEXAGON_VERTICES)):
        p1 = HEXAGON_VERTICES[i]
        p2 = HEXAGON_VERTICES[(i + 1) % len(HEXAGON_VERTICES)]
        cross_product = (p2[0] - p1[0]) * (y - p1[1]) - (p2[1] - p1[1]) * (x - p1[0])
        signs = signs | (cross_product < -1e-9)
    return ~signs

def check_territory(window_fn: Callable,
                    x_test: jax.Array,
                    dominance_threshold: float = 0.8) -> bool:
    """
    输入：
        window_fn(x) → PoU 权重 (N,M)
        x_test       → 测试点 (N,2)
    返回：
        True  → 每个分区都有领地且在其领地内至少有 1 点权重 ≥ dominance_threshold
        False → 否则打印具体信息并返回 False
    """
    weights = window_fn(x_test)                  # (N,M)
    ...
    if len(unique_dominant_indices) < num_partitions:
        missing_indices = jnp.setdiff1d(jnp.arange(num_partitions),
                                        unique_dominant_indices)
        print(f"       - HEALTH CHECK FAILED: "
              f"Partitions {missing_indices} have no territory!")
        return False

    # —— ② 领地是否“够强” —— #
    for i in range(num_partitions):
        weights_in_territory = weights[dominant_partition_indices == i, i]
        if weights_in_territory.size == 0:
            continue
        if jnp.max(weights_in_territory) < dominance_threshold:
            print(f"       - HEALTH CHECK FAILED: Partition {i} is weak "
                  f"(peak weight {jnp.max(weights_in_territory):.3e} "
...
class PoissonHexagonConstantRHS:
    def __init__(self, adf_fn: Callable):
...
class PoissonHexagonComplicatedRHS(PoissonHexagonConstantRHS):
    def __init__(
        self,
        adf_fn,                          # 传入同父类的 approximate distance function
        k_min: float = 2.0,
        k_max: float = 10.0,
        omega: float = jnp.pi,
    ):
        super().__init__(adf_fn=adf_fn)  # 调用父类构造
        self.k_min = float(k_min)
        self.k_max = float(k_max)
        self.omega = omega              # ω 用 jnp.float64，方便后续 JIT
...
def plot_results_on_grid(problem, model, stage_index, n_sub, save_dir, grid_res=201):
    ...
    mask = is_inside_hexagon(grid_points).reshape(grid_res, grid_res)
...
def plot_pou_3d(window_fn, stage_index, n_sub_next, save_dir, grid_res=101):
    ...
    grid_points = jnp.array(np.stack([X.ravel(), Y.ravel()], axis=-1))
    weights_flat = window_fn(grid_points)
    mask = ~np.asarray(is_inside_hexagon(grid_points))
...
def plot_adf_3d(adf_fn, save_dir, grid_res=101):
    ...
    mask = ~np.asarray(is_inside_hexagon(grid_points)).reshape(grid_res, grid_res)
...
def run_hierarchical_fbpinn(key, problem_class, config):
    ...
    print("\n" + "!"*80)
    print("WARNING: Using UN-SCALED Approximate Distance Function.")
    print("         This is for experimental purposes and may lead to numerical instability.")
    print("!"*80 + "\n")
    adf_fn = adf_hexagon

    problem = problem_class(adf_fn=adf_fn)

    print("\n" + "="*80 + "\n===== Computing FEM Reference Solution =====\n" + "="*80)
    U_fem, X_fem, Y_fem, interior_pts, u_flat = solve_poisson_fem_hexagon_fenics(MESH_RESOLUTION)
